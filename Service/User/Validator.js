...
const SUPER_PUBLIC_CHANNEL_ADMIN_ID = "ADMIN";
...
const UserValidator = {
	...
	check: async (id) => await UserService.getUserById(id).then((res) => !!!res),
	...
	validate: async (userId, username) => {
		...
		try {
			...
			if (error instanceof UserNotFoundException) {
				...
				// 		"가입 승인이 되지 않았습니다. 완료 후 로그인 가능합니다."
				// 	);
	...
	checkUserOnPublicChannels: async (userId) => {
		const toCount = (channels) =>
			channels.map((channel) => channel.type == "super_public").length;

		let count = await ChannelService.getChannelsByUserId(userId).then(toCount);

		if (SUPER_PUBLIC_CHANNEL_MAP.size != count)
			for (const [channelId] of SUPER_PUBLIC_CHANNEL_MAP.entries()) {
				await ChannelService.addUserToChannel(channelId, userId);
				await ChannelService.EX_API.addUserToChannel(channelId, userId);
			}
	},
};
